...
class QLearningController(Controller):
    ...
    def discretize(self, bird, pipe) -> Tuple:
        ...
        # Gap center
        gap_center = pipe.gap_bottom_y + (PIPE_GAP)
...
def train(controller, episodes=4):
    for episode in range(episodes):
        best_score = 0

        for pipe_x in range(0, 200, DX_BIN_SIZE):
            for pipe_y in range(100, 400, DY_BIN_SIZE):
                for bird_y in range(0, 512, Y_BIN_SIZE):
                    for vel in range(
                        -16,
                        16,
                    ):
                        score = simmulate_game_state(
                            controller,
                            Pipe(pipe_x, gap_bottom_y=pipe_y),
        ...
        # controller.epsilon = max(0.01, controller.epsilon * 0.995)

        print(f"Episode {episode}, Score: {score}, Epsilon: {controller.epsilon:.3f}")
